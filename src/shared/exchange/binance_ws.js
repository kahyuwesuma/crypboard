...
function startBTCUSDTWS(callback) {
  ...
  ws.on("message", (raw) => {
    ...
    if (lastBtcPrice !== price) {
      lastBtcPrice = price;
      // callback khusus untuk BTCUSDT
      callback({ type: "header", symbol: "BTCUSDT", price: lastBtcPrice });
    }
  });

  ws.on("error", (err) => {
    console.error("[BTCUSDT WS] Error:", err.message);
  });
...
module.exports = { startBinanceWS, stopBinanceWS, getBinanceOrderbook, closeBinanceOrderbookWS, startBTCUSDTWS, stopBTCUSDTWS};